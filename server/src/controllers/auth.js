...
//     const { name, phone, password, email } = req.body  // quăng name, phone, password, email - req.body: lấy dc thông tin của client gửi lên
//     try {
//         if (!name || !phone || !password || !email) return res.status(400).json({ // mã lối client sẽ gửi cho client
//             err: 1,
//             msg: 'Missing inputs !'
//         })
//         const response = await authService.registerService(req.body) // trả về 
...
export const register = asyncHandler(async (req, res) => {
    ...
    return res.json({
        err: !user ? 0 : 1,
        msg: user ? 'Email đã được đăng ký' : 'Hãy check mail để kích hoạt tài khoản',
    })
})

...
export const deleteExpired = async (req, res) => {
    try {
        if (!req.query.eid) return res.status(400).json({
            err: 1,
            msg: 'Missing inputs'
        })
        const response = await postService.deleteExpired(req.query.eid)
        return res.status(200).json(response)
...
export const getDashboard = asyncHandler(async (req, res) => {
    ...
    const [ctpt, ctmb, ctch, nct, tnog, views, postCount, userCount] = await Promise.all([
        db.Post.findAll({
    ...
    return res.json({
        success: [ctpt, ctmb, ctch, nct, tnog, views, postCount, userCount] ? true : false,
        chartData: [ctpt, ctmb, ctch, nct, tnog, views, postCount, userCount]
            ? { ctpt, ctmb, ctch, ...views[0], ...postCount[0], ...userCount[0], nct, tnog }
            : 'Cannot get chart'
    })
})

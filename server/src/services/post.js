...
export const getPostsLimitService = (page, { limitPost, order, ...query }, { priceNumber, areaNumber }) => new Promise(async (resolve, reject) => {
    try {
        ...
        const queries = { ...query }
        // Thêm điều kiện để chỉ lấy các bài đăng chưa được thuê
        query.status = 'ACTIVE';
...
export const updateWishlist = ({ pid, uid }) => new Promise(async (resolve, reject) => {
    try {
        ...
        if (already) {
            ...
            resolve({
                err: response > 0 ? 0 : 1,
                msg: response > 0 ? 'Bạn đã thêm vào bài đăng yêu thích' : 'Something went wrong',
            })
        } else {
            const response = await db.Wishlist.create({ pid, uid })
            resolve({
                err: response ? 0 : 1,
                msg: response ? 'Bạn đã thêm vào bài đăng yêu thích' : 'Something went wrong',
            })
        }
    } catch (error) {
        reject(error)
...
export const deleteExpired = (eid) => new Promise(async (resolve, reject) => {
    try {
        const response = await db.Expired.destroy({ where: { id: eid } })
        resolve({
            err: response > 0 ? 0 : 1,

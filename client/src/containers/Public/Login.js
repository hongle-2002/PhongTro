...
const Login = () => {
    ...
    const location = useLocation() // cung cấp 1 cách dễ dàng để truy cập và sd thông tin về vị trí hiện tại của người dùng
    const dispatch = useDispatch()
    const navigate = useNavigate()
    const { isLoggedIn, msg, update } = useSelector(state => state.auth)

    // Biến lưu thông tin đã được đăng kí hay chưa
